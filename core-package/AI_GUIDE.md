...
If you have the **full repository** checked out, also read `AGENTS.md` (build, tests, conventions). This file is the **portable** copy for zips and minimal deployments.
...
**Required fields** (for compatibility): the line `### TASK-XXX |`, and lines starting with `**Priority**:`, `**Category**:`, `**Created**:`.
...
If you omit the `(id)` segments (for example only `📝 To Do | 🚀 In Progress | ✅ Done`), Void.md cannot parse custom columns. It falls back to built-in default titles, your `##` headings no longer match the internal column IDs, and **tasks may not appear on the board** even though `kanban.md` loads. Fixing it means restoring IDs on the `**Columns**:` line and keeping `##` titles aligned.
...
**Priority**: High | **Category**: Contact | **Assigned**: @user
**Created**: 2026-04-09
...
*Core package – Void.md. For full-repo agent conventions, see `AGENTS.md`.*
*Version: 1.3.3*
